...
// 旗子樣式：建房帶 style → start 回傳該樣式；非法顏色退回預設
const i9 = await connect('I');
const j9 = await connect('J');
send(i9, { type: 'create', name: 'Ivy', style: { color: '#33cc88', seed: 42 } });
const ic = await recv(i9);
send(j9, { type: 'join', code: ic.code, name: 'Jack', style: { color: 'NOTACOLOR', seed: 5 } });
const iStart = await recv(i9);
await recv(j9);
assert(iStart.styles[0].color === '#33cc88' && iStart.styles[0].seed === 42, '建房旗子樣式被保留');
assert(iStart.styles[1].color === '#d62a2a', '非法顏色退回預設（座位 1 紅）');
i9.ws.close();
j9.ws.close();
